...
locals {
  source_bucket_name = "${include.envcommon.locals.account_id}-${include.envcommon.locals.aws_region}-data-source-${include.envcommon.locals.environment}"
...
inputs = merge(
  ...
  {
    name                 = "s3-replication-${include.envcommon.locals.environment}-${include.envcommon.locals.aws_region}"
